...
int main() {
    ...
    while (true) {
        //if interrupts handling inbetween, it might not work properly when more drivers
        //i2c_write_byte('I');
        if(isValidBarcode() == 1){
        ...
        else if (isValidBarcode() == 2)
        {
            ...
            printf("Decoded Barcode: *%c*\n\r", decodeReverse(barcodeDecoded));
            
            //reset barcodeRead
            clearBarcodeRead();

            //reset barcode variables
            barcodeFirstChar = 0;
            barcodeSecondChar = 0;
            barcodeThirdChar = 0;
        }
